...
public class CastTest
{
    ...
    public void Basic()
    {
        ...
        CollectionAssert.AreEqual(Enumerable.Range(0, 10).Cast<int?>().ToArray(),
            SpanEnumerable.Range(0, 10).Cast<int?>().ToArray());
    ...
    public void CrossTest()
    {
        static void Test<T1, T2>(T1[] a)
        {
            ...
            try
            {
                var result = Enumerable.Cast<T2>(a);
                expectedType = result.ElementAt(0)?.GetType();
            }
            ...
            try
            {
                var result = a.AsSpan().Cast<T1, T2>();
                actualType = result.ElementAt(0)?.GetType();
            }
            catch (InvalidCastException)
            {
